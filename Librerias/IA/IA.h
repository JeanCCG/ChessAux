...
// hallar el [1000] del arrr[1000][6]
int minimax(char slots[26][26], int depth, bool player, int points, int arrr[1000][5], int &arrrms)
{ //Player = True
	if (depth == 0)
	{
		return points;
	...
	if (player)
	{
		...
		for (int i = 0; i < 8; ++i)
		{
			for (int j = 0; j < 8; ++j)
			{
						...
						for (int o = 0; o < 8; ++o)
						{
                            ...
                            case PiecesChar::charP1_pawn:
                            {
                                ...
                                for (k = 1; k <= cas; k++)
                                {
                                    if (k == 1)
                                    {
                                        if (p2validpieces(slots[p - 1][o + 1]))
                                        {
                                            movesp1[movesp1m][0] = p - 1;
                                            movesp1[movesp1m][1] = o + 1;
                                            movesp1m++;
                                    ...
                                    if (slots[p - k][o] == PiecesChar::char_free)
                                    {
                                        movesp1[movesp1m][0] = p - k;
                                        movesp1[movesp1m][1] = o;
                                        movesp1m++;
                                    }
                                    else
				...
				{
					availableeat = false;
					for (int p = 0; p < movesm; p++)
					{ // move
						if (p2validpieces(slots[moves[p][0]][moves[p][1]]))
						{ //points
							availableeat = true;
		...
		for (int i = 0; i < 8; ++i)
		{
			for (int j = 0; j < 8; ++j)
			{
				...
				if (availablepiece)
				{
					...
					for (int p = 0; p < movesm; p++)
					{ // move
						if (p1validpieces(slots[moves[p][0]][moves[p][1]]))
						{ //points
							availableeat = true;
